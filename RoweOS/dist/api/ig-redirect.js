...
// iOS universal links intercept instagram.com in ALL iOS browser contexts.
// This page loads on our domain in SFSafariViewController, copies OAuth URL to clipboard.
// User pastes in Safari, signs in, token syncs back via Firestore.
// "Close" dismisses SFSafariViewController, returning to PWA where visibilitychange picks up token.
...
export default function handler(req, res) {
  ...
  res.send('<!DOCTYPE html><html><head>' +
    ...
    '.btn-close{display:none;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.6);font-size:15px;font-weight:600;padding:14px 24px;border-radius:14px;cursor:pointer;width:100%;margin-top:12px;-webkit-appearance:none}' +
    ...
    '<button class="btn-close" id="closeBtn" onclick="closePage()">Close Instagram Setup</button>' +
    ...
    '  setTimeout(function() {' +
    '    document.getElementById("hint").textContent = "Tap the X in the top-left corner to close this window.";' +
    ...
    '      document.getElementById("hint").textContent = "Open Safari, paste the link, and sign in. Then close this window.";' +
    '      document.getElementById("hint").style.color = "#a89878";' +
