...
function handleAuthState(user) {
  ...
  checkUserAccessKey().then(function(keyResult) {
    if (!keyResult.valid) {
      // No valid key - show access key prompt
      console.log('[Auth] No valid access key - showing key prompt');
      if (statusEl) { statusEl.style.color = '#a89878'; statusEl.textContent = 'Signed in as ' + user.email; }
      showAccessKeyPrompt();
      return;
    }
    ...
    // If access key check fails (e.g. rules not deployed), allow access
    hideAuthGate();
    showStartupScreen();
  });
}
...
function generateBetaWelcomeEmail(accessKey, tier) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>'
    ...
    // Content
    + '<div style="background:#111;padding:36px 40px 40px;">'
    + '<h2 style="color:#fff;font-size:22px;font-weight:500;margin:0 0 8px;">Welcome to RoweOS Early Access</h2>'
...
function loadFromFirebaseV2(showNotification, skipModeSync) {
  ...
  return Promise.all([
    ...
    if (convAgentDoc.exists && shouldSyncCategory('brandai_chats')) {
