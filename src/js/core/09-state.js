  ...
  try {
    if (writeOpts.merge !== false) {
      db.doc(fullPath).set(data, { merge: true }).then(function() {
        ...
        // v35.12: MUST be a millisecond epoch string. mergeByTimestamp() runs
        // parseInt() on this value; a locale string like "7/7/2026, 3:45 PM"
        // parsed to 7, which is not NaN, so the date-parse fallback never fired
        // and the drop-stale-local-items branch was permanently bypassed —
        // silently disabling cloud-deletion enforcement on this path.
...
function deleteDBDoc(collectionPath, docId, category) {
  ...
  try {
    ...
    // v35.12: Was an empty catch. A synchronous throw here (malformed path,
    // SDK torn down) left the item tombstoned locally but live in Firestore,
    // causing resurrection on the next pull. Surface it like writeDBDoc does.
    console.warn('[WriteDB] deleteDBDoc sync throw for ' + collectionPath + '/' + docId + ':', e && e.message);
  }
  ...
  queue.forEach(function(entry) {
    ...
    try {
      ...
      if (entry.options && entry.options.action === 'delete') {
        ...
        // v35.12: Honor the original write's merge option. Full-replace writes
        // (merge:false, e.g. writeDBTodos) MUST NOT be replayed as merges, or
        // deletions made while unauthenticated silently resurrect on catch-up.
        var _replayMerge = !(entry.options && entry.options.merge === false);
        batch.set(ref, entry.data, { merge: _replayMerge });
      }
      batchCount++;
      if (batchCount >= 499) {
